 ...
 * (c) MyTribalTotem.com
 *
 * Firmware for JeeNode based totems
 *
 * 
 */
#include <EEPROM.h>
...
#ifdef DEMO
#define BEACON_INTERVAL  10000   /* How often to send beacon */
#else
...
#define SYNC_BYTE        21
#define LED              9       /* Basic LED on JeeNode */
#define RED              9
#define GREEN            9
#define BLUE             9
...
//    Serial.println("grf22 init failed");
  rf12_initialize(31,RF12_433MHZ);
...
//  beacon += "REWRITE";
  ...
  /* Stripped RID to keep packets to 65 byte limit of RF12 lib */
//  beacon += "&rid=";
//  beacon += seed;
...
void flashLED(uint8_t count)
{
  ...
  while (count);
  //turnLED(initLEDState);  /* back to where we started */
}
/*---------------------------------------------------------------------------------------------------------------------*/
void setLED()
{
...
/*---------------------------------------------------------------------------------------------------------------------*/
int rf12_waitRecv(uint8_t* recv_buf, int recv_buf_len,unsigned long timeout)
{
  unsigned long due = millis() + timeout;
  memset(recv_buf,0x0,recv_buf_len);
  while (due > millis() )
  {
    if (rf12_recvDone() && rf12_crc == 0 )
     {
       if (rf12_len > recv_buf_len)
       {
         Serial.println("ERROR - RECV BUFFER OVERRUNE");
         rf12_len = recv_buf_len;
       }
       memcpy(recv_buf,(const void*)rf12_data,rf12_len);   
       return true;      
     } 
  }
  return false;
...
//        calcLED();       
      ...
      flashLED(1); /* flash light on send */   
//      cycleLEDs();   
...
//      if (grf22.sendto((uint8_t*)beaconChars, beacon.length(),RF22_BROADCAST_ADDRESS)) 
      if (rf12_canSend() )
      {
        rf12_sendStart(0,beaconChars,beacon.length());
        {
//        grf22.waitPacketSent();
          rf12_sendWait(2);        //wait for send in standby mode
          calcNextBeacon();
        }
      }
    }
...
//      if (grf22.recv(buf, &len))
      if (rf12_waitRecv(buf,len,random(RECV_WAIT_MIN,RECV_WAIT_MAX)))
      {
